...
def main(seeds, sizes, batch_size: int, fake_submit: bool = True, partition='long'):
    command = "python coin_train.py hp=reproduce_juan wandb.state=enabled wandb.tags=[coin,scalability_v0] wandb.wandb_dir=/home/mila/a/aghajohm/scratch/loqa/wandb  hp.seed={seed} hp.game.width={size} hp.game.height={size} hp.batch_size={batch_size} hp.save_every=1000 hp.save_dir=/home/mila/a/aghajohm/scratch/loqa hp.actor.train.lr_loss_actor=0.001 hp.qvalue.train.lr_loss_qvalue=0.01 hp.actor.train.entropy_beta=0.1 hp.actor.layers_before_gru=2 hp.actor.hidden_size=128 hp.agent_replay_buffer.mode=enabled hp.agent_replay_buffer.capacity=10000 hp.agent_replay_buffer.update_freq=10 hp.just_self_play=True"
    for size in sizes:
        for seed in seeds:
            run_job(command=command.format(seed=seed, size=size, batch_size=batch_size), fake_submit=fake_submit, partition=partition)
