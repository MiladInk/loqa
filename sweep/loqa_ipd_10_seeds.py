...
def run_job(command, fake_submit: bool = True, partition: str = 'long'):
    ...
    if fake_submit:
        ...
        command = f'sbatch --time=0:20:0 --partition={partition} --gres=gpu:a100l:1 --mem=20Gb -c 4 sweep_temp_job.sh'
        os.system(command)

def main(seeds, fake_submit: bool = True, partition='long'):
    commands = [
        (f"python ipd.py wandb.state=enabled "
         f"wandb.tags=[ipd_10_seeds] "
         f"hp=logits hp.seed={seed} "
         f"hp.batch_size=128"
         f" hp.game_length=50 "
         f"hp.qvalue.mode=mean "
         f"hp.qvalue.train.lr_loss_qvalue=1e-2 "
         f"hp.actor.train.lr_loss_actor=1e-3 "
         f"hp.actor.train.entropy_beta=0.0"
         f" hp.agent_replay_buffer.mode=disabled "
         f" hp.op_softmax_temp=0.1")
        for seed in seeds
    ]
