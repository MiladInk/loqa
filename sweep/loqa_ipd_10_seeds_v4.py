...
def main(seeds, fake_submit: bool = True, partition='long'):
    commands = [
        f"python ipd.py wandb.state=enabled wandb.tags=[ipd_10_seeds_v4] hp=logits hp.eval_every=100 hp.batch_size=2048 hp.game_length=50 hp.qvalue.mode=mean hp.qvalue.train.lr_loss_qvalue=1e-2 hp.actor.train.lr_loss_actor=1e-3 hp.actor.train.entropy_beta=0.0 hp.seed={seed} hp.agent_replay_buffer.mode=disabled hp.epsilon_greedy=0.2 hp.qvalue.replay_buffer.mode=disabled"
        for seed in seeds
    ]
