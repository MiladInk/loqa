...
def main(seeds, fake_submit: bool = True, partition='long'):
    commands = [
        f"python ipd.py wandb.state=enabled wandb.tags=[ipd_10_seeds_v2] hp=logits hp.eval_every=100 hp.batch_size=128 hp.game_length=50 hp.qvalue.mode=mean hp.qvalue.train.lr_loss_qvalue=1e-2 hp.actor.train.lr_loss_actor=1e-3 hp.actor.train.entropy_beta=0.0 hp.seed={seed} hp.agent_replay_buffer.mode=disabled hp.epsilon_greedy=0.2"
        for seed in seeds
    ]
